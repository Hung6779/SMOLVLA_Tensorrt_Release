#!/usr/bin/env python3
"""Export a public SmolVLA checkpoint and validate ONNX Runtime.

Camera count, image size, state/action dimensions, and token length are all
read from the checkpoint's config, so this works for both the two-camera
smolvla_libero checkpoint and the three-camera smolvla_base checkpoint.
"""
...
DEFAULT_PROMPT = "pick up the black bowl and place it on the plate"


def input_names(num_cameras: int) -> tuple[str, ...]:
    cameras = tuple(f"camera{i + 1}" for i in range(num_cameras))
    return cameras + ("lang_tokens", "lang_attention_mask", "state", "noise")
...
def policy_dimensions(policy: nn.Module) -> dict[str, int]:
    ...
    image_height, image_width = policy.config.resize_imgs_with_padding
    return {
        ...
        "tokenizer_max_length": int(policy.config.tokenizer_max_length),
    }
...
def cast_policy(policy: nn.Module, dtype: torch.dtype) -> nn.Module:
    """Cast a policy to a reduced dtype the way SmolVLA is designed to run.

    Only `vlm_with_expert` (the VLM and the action expert) runs in the reduced
    dtype: its layers cast activations to their own weight dtype, and it keeps
    attention scores and RoPE in FP32 internally. The projection layers around
    the denoising loop (state_proj, action_in_proj, action_out_proj,
    action_time_mlp_*) and the Euler state x_t stay FP32 (see
    modeling_smolvla.py, `suffix_out.to(torch.float32)` before `action_out_proj`).
    """

    policy.model.vlm_with_expert.to(dtype=dtype)
    return policy


class ExportableSmolVLA(nn.Module):
    """Expose the policy core with explicit images, state, and noise.

    Positional inputs are: one tensor per camera, lang_tokens,
    lang_attention_mask, state, noise (see input_names()).
    """
    ...
    def __init__(self, policy: nn.Module, dimensions: dict[str, int]):
        ...
        self.num_cameras = dimensions["num_cameras"]
        if self.num_cameras < 1:
            raise ValueError("The checkpoint must define at least one camera")
    ...
    def forward(self, *inputs: Tensor) -> Tensor:
        cameras = inputs[: self.num_cameras]
        lang_tokens, lang_attention_mask, state, noise = inputs[self.num_cameras :]
        batch_size = state.shape[0]
        image_mask = torch.ones(batch_size, dtype=torch.bool, device=state.device)
        images = [camera * 2.0 - 1.0 for camera in cameras]
        image_masks = [image_mask] * self.num_cameras
...
def make_inputs(
    policy: nn.Module, dimensions: dict[str, int], seed: int, prompt: str = DEFAULT_PROMPT
) -> tuple[Tensor, ...]:
    ...
    encoded = tokenizer(
        ...
        padding="max_length",
        truncation=True,
        max_length=dimensions["tokenizer_max_length"],
    )
    cameras = tuple(
        torch.rand(
            1,
        ...
        for _ in range(dimensions["num_cameras"])
    )
    return cameras + (
...
def save_reference(directory: Path, names: Sequence[str], inputs: Sequence[Tensor]) -> None:
...
def export_bundle(
    ...
    policy.eval()
    reduced = args.dtype != "float32"
    if reduced:
        cast_policy(policy, getattr(torch, args.dtype))
    ...
    inputs = make_inputs(policy, dimensions, args.seed, args.prompt)
    input_arrays = {
        name: np.ascontiguousarray(value.detach().cpu().numpy())
        for name, value in zip(names, inputs, strict=True)
    }
    image_shape = (1, 3, dimensions["image_height"], dimensions["image_width"])
    token_shape = (1, dimensions["tokenizer_max_length"])
    expected_inputs = {
        **{f"camera{i + 1}": (image_shape, np.float32) for i in range(dimensions["num_cameras"])},
        "lang_tokens": (token_shape, np.int64),
        "lang_attention_mask": (token_shape, np.int64),
    ...
    save_reference(reference_dir, names, inputs)
    ...
    try:
        ...
        ort_output = session.run(["actions"], input_arrays)[0]
    except Exception as error:  # ONNX Runtime CPU lacks some FP16/BF16 kernels
        if not reduced:
            raise
        ort_error = str(error)[:500]
    ...
    report = {
        ...
        "opset": args.opset,
        "dtype": args.dtype,
        "execution_providers": session.get_providers() if session else [],
        "derived_configuration": dimensions,
        "interface": session_interface(session) if session else {},
        ...
        "max_absolute_error": None if difference is None else float(np.abs(difference).max()),
        "mean_absolute_error": None if difference is None else float(np.abs(difference).mean()),
        "atol": args.atol,
        "rtol": args.rtol,
        "passed": passed,
        "ort_validation_skipped_reason": ort_error,
        "versions": {
...
def main() -> None:
    ...
    parser.add_argument("--prompt", default=DEFAULT_PROMPT, help="instruction used for the reference inputs")
    parser.add_argument("--opset", type=int, default=17)
    parser.add_argument(
        "--dtype",
        choices=["float32", "float16", "bfloat16"],
        default="float32",
        help="compute dtype of the VLM+expert; the graph inputs/outputs stay FP32",
    )
    parser.add_argument("--atol", type=float, default=None, help="default 1e-3 (2e-2 if reduced dtype)")
    parser.add_argument("--rtol", type=float, default=None, help="default 1e-3 (2e-2 if reduced dtype)")
    args = parser.parse_args()

    if args.opset < 1:
        raise ValueError("--opset must be positive")
    default_tolerance = 1.0e-3 if args.dtype == "float32" else 2.0e-2
    ...
    if not (checkpoint / "config.json").is_file() or not (checkpoint / "model.safetensors").is_file():
        raise ValueError("--checkpoint must be a SmolVLA directory with config.json and model.safetensors")
    ...
    if passed is None:
        print(
            "EXPORTED: ONNX Runtime CPU could not run this dtype, so the export was "
            "not validated against ONNX Runtime (see validation.json); validate with "
            "TensorRT instead"
        )
    else:
        print(
